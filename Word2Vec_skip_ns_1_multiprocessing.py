...
class Word2Vec(object):
    ...
    def Main(self):
        ...
        for i in range(self.num_thread):
            p = mp.Process(target=TrainModelThread, args=(self.num_thread, self.word_count_actual, i,
                                                          ...
                                                          self.exp_table_size, self.negative, self.table,
                                                          self.table_size))
            p.start()
    ...
    def InitUnigramTable(self):
        print('init table start')
        power = 0.75
        self.table = np.zeros(self.table_size, dtype=np.int)
        train_words_pow = np.sum(np.power(self.vocab_cn, power))
        i = 0
        d1 = np.power(self.vocab_cn[i], power) / train_words_pow
        for a in range(self.table_size):
            self.table[a] = int(i)
            if a / float(self.table_size) > d1:
                i += 1
                d1 += np.power(self.vocab_cn[i], power) / train_words_pow
            if i >= self.vocab_size:
                i = self.vocab_size - 1

        print('init table stop')
